...
function Projectile(url, target, caller, skill){
  ...
  this.x = caller.x || players[0].x;
  this.y = caller.y || players[0].y;
  this.animStart = frameTime;
  this.speed = 30*dist(players[0], target);
  this.state = 1;
  this.animationSpeed = 120;
  players[0].exhausted = this.exhausted;
  ...
  this.hit = function(){
    target.takeDamage(players[0], this.damage);
  }
}
function Fireball(target, skill){
    this.damage = Math.round(Math.random()) * players[0].intelligence + 85;
    players[0].manaCur -= 200;
    this.exhausted = frameTime + 500;
    players[0].skills[0].cooldown = frameTime + 2000;
    Projectile.call(this, 'img/projectile_new.png', target, players[0], skill);
}
function RocketLauncher(target, skill){
    this.damage = this.damage = Math.random() * 0.7 * (players[0].intelligence + 45) +10;
    players[0].manaCur -= 200;
    this.exhausted = frameTime + 500;
    players[0].skills[1].cooldown = frameTime + 750;
    Rocket.call(this, 'img/missile.png', target, players[0], skill);
}
function Rocket(url, target, caller, skill){
  ...
  this.puffs = [];
  this.homing = true;
  players[0].exhausted = frameTime + 100;
  this.update = function(){
    if(this.homing) {
      if(target.isDead){
        this.homing = false;
...
function Explosion(){
  ...
  this.animStart = frameTime;
  this.animationSpeed = 120;
  players[0].exhausted = frameTime + 1500;
  this.update = function(){
    this.animationFrame = Math.floor((frameTime - this.animStart) / this.animationSpeed);
    if(this.animationFrame > 6)delete missiles[this.id];
  }
